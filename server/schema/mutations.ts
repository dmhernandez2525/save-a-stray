...
import WaitlistType from './types/waitlist_type';
  ...
  fields: (): GraphQLFieldConfigMap<unknown, unknown> => ({
    ...
    removeFromWaitlist: {
      ...
      async resolve(_, args: { _id: string }) {
        const entry = await WaitlistModel.findById(args._id);
        if (!entry) return null;
        await WaitlistModel.deleteOne({ _id: args._id });
        // Shift positions down for remaining entries
        await WaitlistModel.updateMany(
          { animalId: entry.animalId, status: 'waiting', position: { $gt: entry.position } },
          { $inc: { position: -1 } }
        );
        return entry;
      }
