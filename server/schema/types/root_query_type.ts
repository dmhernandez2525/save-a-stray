  ...
  fields: (): GraphQLFieldConfigMap<unknown, unknown> => ({
    ...
    animalWeightHistory: {
      ...
      resolve(_, args: { animalId: string }) {
        return WeightRecord.find({ animalId: args.animalId }).sort({ recordedAt: -1 });
      }
    },
    shelterWeightRecords: {
      type: new GraphQLList(WeightRecordType),
      args: { shelterId: { type: new GraphQLNonNull(GraphQLID) } },
      resolve(_, args: { shelterId: string }) {
        return WeightRecord.find({ shelterId: args.shelterId }).sort({ recordedAt: -1 });
      }
    },
    latestAnimalWeight: {
      ...
      args: { animalId: { type: new GraphQLNonNull(GraphQLID) } },
      resolve(_, args: { animalId: string }) {
        return WeightRecord.findOne({ animalId: args.animalId }).sort({ recordedAt: -1 });
      }
    }
  })
});
