...
- Add query protections (depth limit + complexity limit) and cursor pagination.
- Add custom scalar validation (`Email`, `URL`, `Date`) for mutation safety.
- Normalize GraphQL errors with stable error codes and safe client messaging.
...
- Add validation rules for max query depth and max query complexity.
...
- Generate connection query TypeScript contracts via `npm run schema:types`.
- Publish generated types to:
...
| 1.1.0   | Feb 15, 2026 | Codex  | Added depth/complexity limits, scalar validation, cursor pagination, error formatting, and schema/type generation outputs |
