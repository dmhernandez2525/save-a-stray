...
describe('Waitlist Management API', () => {
  it('should have WaitlistType with correct fields', () => {
    const WaitlistType = require('../server/schema/types/waitlist_type').default;
    const fields = WaitlistType.getFields();
    expect(fields._id).toBeDefined();
    expect(fields.animalId).toBeDefined();
    expect(fields.shelterId).toBeDefined();
    expect(fields.userId).toBeDefined();
    expect(fields.userName).toBeDefined();
    expect(fields.userEmail).toBeDefined();
    expect(fields.userPhone).toBeDefined();
  ...
  it('should have animalWaitlist query', () => {
    const RootQueryType = require('../server/schema/types/root_query_type').default;
    const fields = RootQueryType.getFields();
    expect(fields.animalWaitlist).toBeDefined();
    const args = fields.animalWaitlist.args.map(a => a.name);
    expect(args).toContain('animalId');
  });

  it('should have shelterWaitlists query', () => {
    const RootQueryType = require('../server/schema/types/root_query_type').default;
    const fields = RootQueryType.getFields();
    expect(fields.shelterWaitlists).toBeDefined();
    const args = fields.shelterWaitlists.args.map(a => a.name);
    expect(args).toContain('shelterId');
  });

  it('should have joinWaitlist mutation', () => {
    const mutation = require('../server/schema/mutations').default;
    const fields = mutation.getFields();
    expect(fields.joinWaitlist).toBeDefined();
    const argNames = fields.joinWaitlist.args.map(a => a.name);
    expect(argNames).toContain('animalId');
    expect(argNames).toContain('shelterId');
    expect(argNames).toContain('userName');
    expect(argNames).toContain('userEmail');
    expect(argNames).toContain('userPhone');
    expect(argNames).toContain('notes');
  });

  it('should have removeFromWaitlist mutation', () => {
    const mutation = require('../server/schema/mutations').default;
    const fields = mutation.getFields();
    expect(fields.removeFromWaitlist).toBeDefined();
    const argNames = fields.removeFromWaitlist.args.map(a => a.name);
    expect(argNames).toContain('_id');
  });

  it('should have updateWaitlistStatus mutation', () => {
    const mutation = require('../server/schema/mutations').default;
    const fields = mutation.getFields();
    expect(fields.updateWaitlistStatus).toBeDefined();
    const argNames = fields.updateWaitlistStatus.args.map(a => a.name);
    expect(argNames).toContain('_id');
    expect(argNames).toContain('status');
  });

  it('should return WaitlistType from joinWaitlist mutation', () => {
    const mutation = require('../server/schema/mutations').default;
    const WaitlistType = require('../server/schema/types/waitlist_type').default;
    const fields = mutation.getFields();
    expect(fields.joinWaitlist.type).toBe(WaitlistType);
  });

  it('should return WaitlistType from removeFromWaitlist mutation', () => {
    const mutation = require('../server/schema/mutations').default;
    const WaitlistType = require('../server/schema/types/waitlist_type').default;
    const fields = mutation.getFields();
    expect(fields.removeFromWaitlist.type).toBe(WaitlistType);
  });

  it('should return WaitlistType from updateWaitlistStatus mutation', () => {
    const mutation = require('../server/schema/mutations').default;
    const WaitlistType = require('../server/schema/types/waitlist_type').default;
    const fields = mutation.getFields();
    expect(fields.updateWaitlistStatus.type).toBe(WaitlistType);
  });
});
